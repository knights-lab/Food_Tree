...
make.food.tree(nodes_fn="data/NodeLabelsMCT.txt", food_database_fn="data/MCT/ASA24Database.txt", 
...
make.food.tree(nodes_fn="data/NodeLabelsMCT.txt", 
               ...
               output_tree_fn="output/mct.reduced.tree.txt", 
               output_taxonomy_fn = "output/mct.reduced.taxonomy.txt")


# this makes the standard food otu table with data in gram weights of food
make.food.otu(food_records_fn="data/MCT/dietrecords.txt", food_record_id = "X.SampleID", food_taxonomy_fn="output/mct.reduced.taxonomy.txt", 
                output_fn = "output/mct.food.otu.txt")

# this makes the food otu table with data in grams of fiber per food
make.fiber.otu(food_records_fn="data/MCT/dietrecords.txt", food_record_id = "X.SampleID", food_taxonomy_fn="output/mct.reduced.taxonomy.txt", 
               output_fn = "output/mct.fiber.otu.txt")



# this makes the food otu table as dehydrated grams per kcal
make.dhydrt.otu(food_records_fn="data/MCT/dietrecords.txt", food_record_id = "X.SampleID", food_taxonomy_fn="output/mct.reduced.taxonomy.txt", 
